...
static hpa_t litevm_mmu_alloc_page(struct litevm_vcpu *vcpu, u64 *parent_pte)
{
	...
	page = list_entry(vcpu->free_pages.next, struct litevm_mmu_page, link);
	list_del(&page->link);
	list_add(&page->link, &vcpu->litevm->active_mmu_pages);
...
static void page_header_update_slot(struct litevm *litevm, void *pte, gpa_t gpa)
{
	...
	struct litevm_mmu_page *page_head = page_header(__pa(pte));
...
static int alloc_mmu_pages(struct litevm_vcpu *vcpu)
{
	...
	for (i = 0; i < LITEVM_NUM_MMU_PAGES; i++) {
		...
		if ((page = alloc_page(GFP_LITEVM_MMU)) == NULL)
			goto error_1;
...
void litevm_mmu_slot_remove_write_access(struct litevm *litevm, int slot)
{
	...
	list_for_each_entry(page, &litevm->active_mmu_pages, link) {
		int i;
		u64 *pt;
